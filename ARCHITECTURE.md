...
| type | 用途 | params 關鍵欄位 |
|------|------|----------------|
| `detect` | OCR 偵測文字，未命中則中斷規則 | `text`, `roi`, `match_mode`, `fuzzy_threshold`, `on_fail` |
| `match_image` | 圖示模板比對，未命中則中斷規則 | `template`, `roi`, `threshold` |
...
- **`on_fail` 只保留 `"key"` 和 `"stop"`**：不再有 `retry_from`
