...
| `condition_list` | 多條件分支：依序匹配 conditions，命中則執行動作 | `conditions`（含 detect/action/on_match）, `advance_on_no_match`, `loop` |
...
- `_migrate_condition_list_to_step()` 偵測 `use_condition_list=True` + `condition_list` 欄位時，自動將其轉為 `steps` 中的一個 `condition_list` Step（v0.0.12）
...
- **設為 True 的步驟**：`click`、`key`、`notify`、`scroll`、`drag`（這些步驟代表「已執行動作」）；`condition_list` 在 `on_match="next_step"` 或 `on_match="stop"` 時設為 True
- **不設 True 的步驟**：`detect`、`match_image`、`compare`、`wait`、`jump`（僅檢查或等待，非動作）；`condition_list` 的 `on_match="repeat"` 不設 True
