...
| type | 用途 | params 關鍵欄位 |
|------|------|----------------|
| `detect` | OCR 偵測文字，未命中則中斷規則 | `text`, `roi`, `match_mode`, `fuzzy_threshold`, `on_fail`（stop/key/skip/jump） |
| `match_image` | 圖示模板比對，未命中則中斷規則 | `template`, `roi`, `threshold`, `on_fail`（stop/key/skip/jump） |
...
- **`on_fail` 支援四種值**：`"stop"`（預設，中斷規則）、`{"action":"key","key":"..."}`（發送按鍵後繼續）、`{"action":"skip","skip_to":N}`（跳至第 N 步繼續）、`{"action":"jump","rule_id":"..."}`（跳到另一規則）。不再有 `retry_from`
- **match_image 與 detect 對稱但不同實作**：`match_image` 使用 OpenCV `matchTemplate` + NMS，傳回 `MatchResult`（具備 `center_x`/`center_y`/`text` 介面，與 `OcrResult` 相容），click handler 不需區分來源
- **capture_size 自動縮放**：當任務 JSON 頂層有 `capture_size` 時，`match_image` 根據當前視窗尺寸計算比例 → 只跑單一 scale（精確縮放），效能最佳；無 `capture_size` 則回退多尺度 (0.8~1.2) 窮舉搜尋（較慢但相容舊任務）
...
1. **`_FIELD_DEFAULTS`** — 加入預設值，確保讀取舊 JSON 不炸裂
2. **`_dict_to_rule()`** — 加入讀取邏輯（含型態轉換與 sanitize）
3. **`_rule_to_dict()`** — 若該欄位不應持久化，在此 `pop()`
