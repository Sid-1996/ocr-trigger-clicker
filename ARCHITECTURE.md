...
v0.1.0 起改為統一步驟系統（Step System），不再區分觸發規則／比較規則。
...
| type | 用途 | params 關鍵欄位 |
|------|------|----------------|
| `detect` | OCR 偵測文字，未命中則中斷規則 | `text`, `roi`, `fuzzy`, `fuzzy_threshold`, `cooldown_ms`, `trigger_mode`, `max_triggers` |
...
v0.1.0 起採用步驟執行模型。`_run_rule(rule, ocr_results, img)` 依序執行 `rule.steps`，
