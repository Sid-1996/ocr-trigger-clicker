...
| `core/04_rule_engine.py` | 規則模型 + 步驟系統 + 任務管理 | `Rule`, `Step`, `Metric`, `Round`, `load_rules()`, `save_rules()`, `_migrate_v1_to_v2()` |
| `core/05_main_loop.py` | 主偵測迴圈（步驟執行模型） | `MainLoop` class, `StepContext`, `StepResult` |
...
| type | 用途 | params 關鍵欄位 |
|------|------|----------------|
| `detect` | OCR 偵測文字，未命中則中斷規則 | `text`, `roi`, `match_mode`, `fuzzy_threshold`, `cooldown_ms`, `trigger_mode`, `max_triggers`, `on_fail` |
| `click` | 滑鼠點擊 | `target`（`text_center`/`custom`）、`x`, `y`, `button`, `random_offset` |
| `key` | 鍵盤按鍵 | `key`（AHK 格式） |
| `wait` | 固定等待 | `ms` |
| `wait_rule` | 等待另條規則觸發後才繼續 | `rule_id` |
| `collect_rounds` | 多輪收集數值、比較選最佳 | `rounds`(list), `primary_metric_index`, `confirm_action`, `on_all_fail` |
| `jump` | 無條件跳轉觸發另條規則 | `rule_id` |

### 不持久化的執行期欄位

| 欄位 | 型態 | 說明 |
|------|------|------|
| `trigger_count` | int | 已觸發次數 |
| `last_trigger_time` | float | 最後觸發時間（`time.monotonic`） |

以上兩個欄位在 `_rule_to_dict()` 中以 `pop()` 移除，不寫入 JSON。
...
v0.0.2 起採用步驟執行模型。`_run_rule(rule, ocr_results, img)` 依序執行 `rule.steps`，
每個步驟由對應的 handler 處理，handler 回傳 `StepResult(action, rule_id)` 控制流程。
...
```
                   ...
                   └────────┬─────────┘
                            │
                   ┌────────▼─────────┐
                   │  依序處理各規則   │  _process_rules() → _run_rule()
                   │  (enabled 檢查、  │
                   │   速率限制)       │
                   └────────┬─────────┘
                            │
                   ┌────────▼─────────┐
                   │  _run_rule(rule)  │  StepContext(ocr_results, img)
                   │  逐 step 執行：   │
                   │  ┌─────────────┐  │
                   │  │ detect      │──│── OCR 比對，未命中 → stop
                   │  │ click       │  │    命中 → ctx.matched_text 傳遞
                   │  │ key         │  │    → AHK 發送指令
                   │  │ wait        │  │    → time.sleep()
                   │  │ wait_rule   │  │    → 檢查依賴規則觸發次數
                   │  │ collect_rds │  │    → 多輪收集、比較、選最佳
                   │  │ jump        │  │    → 跳轉至另一規則
                   │  └─────────────┘  │
                   │  任一 step 回傳   │
                   │  stop/jump 即中斷 │
...
| 前景保護 | `05_main_loop.py` | 固定強制行為：僅在目標視窗為前景時才執行點擊，非前景時靜默等待 |
