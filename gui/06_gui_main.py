...
class _ClickStepForm(QWidget):
    def __init__(self, parent_list, step, idx, pick_cb):
        ...
        self._hold_ms.setValue(p.get("hold_ms", 0))
    ...
    def _populate(self, entries: list[dict]):
        ...
        for row, e in enumerate(entries):
            ...
            for col, text in enumerate(items):
                ...
                elif result in ("completed", "triggered"):
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self._task_menu_btn = QPushButton("▾ " + T("main.task_actions"))
        self._task_menu_btn.setToolTip(T("tooltip.task_actions"))
        _task_menu = QMenu(self)
        _task_menu.addAction(T("main.task_new"), self._on_task_new)
        _task_menu.addAction(T("main.task_rename"), self._on_task_rename)
        _task_menu.addAction(T("main.task_delete"), self._on_task_delete)
        _task_menu.addSeparator()
        _task_menu.addAction(T("main.import_task"), self._on_task_import)
        _task_menu.addAction(T("main.export_task"), self._on_task_export)
        _share_menu = _task_menu.addMenu(T("main.share_task"))
        ...
        self._task_menu_btn.setMenu(_task_menu)
        toolbar.addWidget(QLabel(T("main.task")))
        toolbar.addWidget(self._task_combo)
        toolbar.addWidget(self._task_menu_btn)
        ...
        self._more_btn.setToolTip(T("tooltip.more"))
        _more_menu = QMenu(self)
        _more_menu.addAction(T("main.sponsor"), self._open_sponsor)
        _more_menu.addAction(T("main.settings"), self._open_settings)
        _more_menu.addAction(T("main.about"), self._show_about)
        _more_menu.addAction(T("main.guide"), self._open_guide)
        _more_menu.addAction(T("main.update"), lambda: self._check_version(force=True))
    ...
    def _show_group_selection_dialog(self) -> Optional[list[str]]:
        ...
        buttons.button(QDialogButtonBox.StandardButton.Ok).setText(T("dialog.group_ok"))
        buttons.accepted.connect(dialog.accept)
        buttons.rejected.connect(dialog.reject)
        layout.addWidget(buttons)
    ...
    def _update_edit_enabled(self, enabled: bool):
        ...
        self._task_menu_btn.setEnabled(enabled)
