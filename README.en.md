...
| 🎮 | **Background (daemon) mode** | PrintWindow capture + Frida-injected clicks — run with the window minimized or covered, zero cursor disturbance, without stealing focus (most Unity games don't support background mode — that's a game-engine limitation, not a tool issue) |
