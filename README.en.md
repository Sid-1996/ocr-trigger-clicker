...
| 🖼️ | **Find buttons with images** | No text? Match by image — faster than reading text; captured images can be **trimmed** to the exact area needed |
...
| 🎮 | **Background mode** | PrintWindow capture + Frida-injected clicks & keys — the window can be covered by other windows (but **not minimized**), zero cursor disturbance, without stealing focus (most Unity games don't support background mode — that's a game-engine limitation, not a tool issue) |
