...
- **新增「混合模式」**：設定 → 一般 → 互動方法 → 混合模式（後台偵測＋前景操作）。平時以後台截圖辨識、零干擾；偵測到需要操作時，短暫把遊戲帶到前景做物理點擊/按鍵，完成後自動復原你原本的前景視窗與滑鼠位置。適合不支援後台注入（Frida）的遊戲——例如遊戲只吃前景操控、正在自動戰鬥爬主線，只有過關時要手動點「下一關」：寫好規則後就能掛著去做別的事，畫面一出現「下一關」，工具自動切回去點擊並復原你原本的狀態。因每次操作會短暫搶焦點，較適合低頻動作的任務。
