...
- F8 群組選擇新增「不再詢問」選項，可在設定頁重新開啟詢問
- 錄製轉換延時改用錄製操作專屬設定（穩定性優先 500ms），與全域延時分離，互不干擾

#### 修正

- 未歸類群組名稱改走多語系顯示，切換語言即時生效
...
- 群組確認彈窗改為雙欄排列＋捲動區域＋全選/全不選，多群組時不再過長

#### 修正

- 群組規則異動後可能被靜默跳過的問題——移除「下次不再提醒」記憶功能，每次啟用群組都會再次確認
- 啟動時若「未歸類」群組有啟用中的規則，會提示不會執行；新增規則不再落入未歸類
- 長等待（wait 步驟／動作後延遲）不再誤報「偵測執行太慢」

## [v0.4.1] - 2026-08-26

### 給開發者

- 發版沙箱驗證用版本：與 v0.4.0 同內容重打包，用於確認 Velopack delta 自動產生、`releases.win.json` 更新與 `-FeedTest` 全流程（無使用者可見變更）。
...
**請下載 OCRTriggerClicker-win-Setup.exe**（本版起以安裝版為主，自動更新；亦提供 OCRTriggerClicker-win-Portable.zip 可攜版）：
