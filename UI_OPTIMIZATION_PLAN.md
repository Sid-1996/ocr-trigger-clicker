...
| # | 項目 | 為什麼需要 | 複雜度 |
|---|------|-----------|--------|
| A | 步驟顏色 | 圖標已有但無顏色區分，同類型步驟難以快速辨識 | 低 |
| B | 執行狀態完整化 | 現有只有 running 指標，缺少 failed/completed 狀態 | 中 |
| C | AHK 持久指示器 | 暫態訊息容易被忽略，用戶無法感知斷線 | 低 |
| D | 參數即時校驗 | 完全沒有，錯誤保存後才發現 | 中 |
| E | Del 刪除步驟 | 規則已有 Del，步驟沒有，需用滑鼠點選刪除按鈕 | 低 |
| F | ROI/click picker 高 DPI | 後端已處理 DPI，但前端 overlay 缺少縮放補償 | 中 |
